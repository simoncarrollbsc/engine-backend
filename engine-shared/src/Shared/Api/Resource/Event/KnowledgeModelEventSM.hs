...
instance ToSchema AddKnowledgeModelEventDTO where
  declareNamedSchema = simpleToSchema'' "_addKnowledgeModelEventDTO" "type" (toDTO a_km1)

instance ToSchema EditKnowledgeModelEventDTO where
  declareNamedSchema = simpleToSchema'' "_editKnowledgeModelEventDTO" "type" (toDTO e_km1)
