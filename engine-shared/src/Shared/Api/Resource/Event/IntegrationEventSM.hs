...
instance ToSchema AddIntegrationEventDTO where
  declareNamedSchema = simpleToSchema'' "_addIntegrationEventDTO" "type" (toDTO a_km1_iop)

instance ToSchema EditIntegrationEventDTO where
  declareNamedSchema = simpleToSchema'' "_editIntegrationEventDTO" "type" (toDTO e_km1_iop)

instance ToSchema DeleteIntegrationEventDTO where
  declareNamedSchema = simpleToSchema'' "_deleteIntegrationEventDTO" "type" (toDTO d_km1_iop)
